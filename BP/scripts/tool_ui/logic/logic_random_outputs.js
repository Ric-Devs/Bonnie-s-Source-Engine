...
export function logicRandomOutputsUI(player, blockEntry, options) {
	...
	addDecorativeSection(form, "�1�1Logic Random Outputs");
	...
	form.show(player).then((response) => {
		...
		if (range < 1 || range > 10) {
			sendUiError(player, "Random Chance Range must be between 1 and 10.");
			return;
		}

		if (runOnce && parseInt(randomnessInterval) > 0 && !runOnInput) {
			sendUiError(player, "Run once and Randomness Interval are mutually exclusive when Run on input is off.");
			return;
		}
		...
		sendUiSaved(player, "Logic Random Outputs", name);
	});
}