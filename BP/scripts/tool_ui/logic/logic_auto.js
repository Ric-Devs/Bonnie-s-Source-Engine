...
export function logicAutoUI(player, blockEntry, options) {
	...
	addDecorativeSection(form, "�1�1Logic Auto");
	...
	form.show(player).then((response) => {
		...
		if (shouldAddAutomation && autoRunOnce && autoRunPeriodically) {
			sendUiError(player, "Run once and Run periodically are mutually exclusive.");
			return;
		}

		if (shouldAddAutomation && automations.length >= 16) {
			sendUiError(player, "Maximum of 16 automations per block.");
			return;
		}
		...
		sendUiSaved(player, "Logic Auto", name);
	});
}