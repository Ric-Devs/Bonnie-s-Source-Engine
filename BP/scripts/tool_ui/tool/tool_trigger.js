...
export function triggerToolUI(player, blockEntry, options) {
	...
	addDecorativeSection(triggerForm, "§1§1Trigger Tool");
	...
	triggerForm.show(player).then((response) => {
		...
		if (shouldAddOutput) {
			...
			if (!isOutputTargetSupportedByBlockType(outputTargetProperty, selectedTargetType)) {
				sendUiError(player, `${getOutputTargetLabel(outputTargetProperty)} is not valid for target type ${formatBlockType(selectedTargetType)}.`);
				return;
			}
		...
		if (isBlockedTriggerCommand(runCommand)) {
			sendUiError(player, "Blocked command: /op and /deop are blacklisted from Trigger blocks.");
			return;
		}
		...
		sendUiSaved(player, "Trigger", blockEntry.data.name);
	});
}