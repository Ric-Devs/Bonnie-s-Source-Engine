...
export function areaPortalToolUI(player, blockEntry, onSave) {
    ...
    addDecorativeSection(portalForm, "§1§1Area Portal Tool");
    ...
    portalForm.show(player).then((response) => {
        ...
        if (!blockEntry.data.destination && !blockEntry.data.destinationBlock) {
            sendUiError(player, "You must specify either a Destination or Destination Block.");
            return;
        }

        // Call the save callback
        if (onSave) onSave(blockEntry);
        sendUiSaved(player, "Area Portal", blockEntry.data.name);
    });
}