...
export function gameNametagUI(player, blockEntry, onSave) {
    ...
    addDecorativeSection(nametagForm, "§1§1Game Nametag Block");
    ...
    nametagForm.show(player).then((response) => {
        ...
        if (addOutput) {
            ...
            if (!isOutputTargetSupportedByBlockType(outputTargetProperty, selectedTargetType)) {
                sendUiError(player, `${getOutputTargetLabel(outputTargetProperty)} is not valid for target type ${selectedTargetType.replace(/^brr:/, "") || "Unknown"}.`);
                return;
            }
        ...
        sendUiSaved(player, "Game nametag", blockEntry.data.name);
    });
}